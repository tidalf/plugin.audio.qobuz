...
class TagProduct(ITag):
    ...
    def parse_json(self, p):
        #info(self, pprint.pformat(p))
        self.auto_parse_json(p)
#        if 'album' in p:
#            print "Found album in product"
#            album_tag = TagAlbum(p['album'])
#            print "album artist: " + album_tag.getArtist()
#            self.add_child(album_tag)
        if 'image' in p:
        ...
        if 'genre' in p:
            ...
            if genre:
                self.add_child(tag)
#        if 'composer' in p:
#            print "Found composer in track"
#            tag_composer = TagComposer(p['composer'])
#            print tag_composer.name
#            self.add_child(tag_composer)
#        if 'interpreter' in p:
#            print "Found interpreter in track"
#            tag_interpreter = TagInterpreter(p['interpreter'])
#            print tag_interpreter.name
#            self.add_child(tag_interpreter)
            
        if 'artist' in p and p['artist']:
            artist = ''
            if 'name' in p['artist']:
