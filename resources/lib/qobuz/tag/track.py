...
from album import Tag_album
from interpreter import Tag_interpreter
from composer import Tag_composer
...
class Tag_track(ITag):
    ...
    def parse_json(self, p):
        ...
        if 'album' in p:
            data = Tag_album(p['album'])
            self.add_child( data )
        if 'interpreter' in p:
            data = Tag_interpreter(p['interpreter'])
            self.add_child( data )
        if 'composer' in p:
            data = Tag_composer(p['composer'])
            self.add_child( data )
        if 'image' in p:
            data = Tag_image(p['image'])
            self.add_child( data )
